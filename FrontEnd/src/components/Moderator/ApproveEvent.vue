  ...
  methods: {
    ...
    async fetchSuggEvents() {
      try {
        ...
        const response = await fetch(import.meta.env.VITE_APP_API_URL + '/suggEvents/get', {
          method: 'GET',
          headers: {
        ...
        events = await Promise.all(events.map(async (event) => {
          try {
            const evalResponse = await fetch(import.meta.env.VITE_APP_API_URL + '/evaluation/get', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
    ...
    async approveEvent(event) {
      try {
        ...
        const response = await fetch(import.meta.env.VITE_APP_API_URL + '/convertEvent/sug_to_off', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
